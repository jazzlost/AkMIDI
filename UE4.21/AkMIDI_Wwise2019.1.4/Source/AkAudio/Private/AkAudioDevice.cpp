	...
	if (m_bSoundEngineInitialized && CallbackManager)
	{
		...
		if (pPackage)
		{
			playingID = AK::SoundEngine::PostEvent(
				TCHAR_TO_AK(*in_EventName), in_gameObjectID, pPackage->uUserFlags | AK_EndOfEvent, &FAkComponentCallbackManager::AkComponentCallback, pPackage, in_ExternalSources.Num(), const_cast<AkExternalSourceInfo *>(in_ExternalSources.GetData()));
